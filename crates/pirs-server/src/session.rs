//! Session manager: append-only JSONL conversation trees in version 3 of pi's
//! session format, plus a per-entry `seq`.
//!
//! Every non-header line is an entry with `id` / `parentId` / `timestamp` /
//! `seq` forming a tree. The "leaf" is the current position; appending creates
//! a child of the leaf, branching moves the leaf to an earlier entry.
//! `build_session_context()` resolves the active path (honouring compaction)
//! into the message list handed to the LLM.
//!
//! `seq` is what D-06 rests on: it starts at 1 and increases by one per
//! appended entry within a file, so `subscribe … since` can replay the log as
//! the loop's event stream. A file written by pi has no `seq`; those entries
//! are numbered in file order when the file is read, and the next append
//! continues from the highest number seen.
//!
//! Files live under `~/.pirs/sessions/<encoded cwd>/`; `PIRS_HOME` moves the
//! whole directory, and `dirs::home_dir()` reads `$HOME`, so a test can
//! redirect either.
...
use pirs_protocol::{ConversationInfo, ServerPath};
...
const APP_NAME: &str = "pirs";
const ENV_AGENT_DIR: &str = "PIRS_HOME";
...
pub struct SessionEntry {
    ...
    /// ISO-8601 timestamp.
    pub timestamp: String,
    /// Position in the file: 1 for the first entry, +1 per append. This is the
    /// `seq` a client replays from (D-06). Entries read from a file that has
    /// none (one pi wrote) are numbered in file order.
    pub seq: u64,
    pub kind: EntryKind,
}
...
impl SessionEntry {
    ...
    fn from_value(value: Value) -> Result<Self> {
        ...
        // 0 means "the file did not say"; `number_entries` fills it in.
        let seq = obj.remove("seq").as_ref().and_then(Value::as_u64).unwrap_or(0);
        Ok(SessionEntry { id, parent_id, timestamp, seq, kind: EntryKind::from_object(obj) })
    }
}

impl Serialize for SessionEntry {
    // Manual so the line layout matches pi with `seq` appended to the fixed
    // prefix: type, id, parentId, timestamp, seq, payload.
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error> {
        ...
        for (k, v) in &payload {
            if !matches!(k.as_str(), "type" | "id" | "parentId" | "timestamp" | "seq") {
...
impl SessionInfo {
    /// The protocol's view of this conversation (`loop.list`, `pirs --list`).
    /// `updated` is `modified` in Unix milliseconds.
    pub fn to_conversation_info(&self) -> ConversationInfo {
        ConversationInfo {
            id: self.id.clone(),
            name: self.name.clone(),
            cwd: ServerPath::from(self.cwd.clone()),
            path: ServerPath::from(path_string(&self.path)),
            updated: self.modified.timestamp_millis().max(0) as u64,
        }
    }
}
...
/// `$PIRS_HOME` or `~/.pirs`: the server's configuration and state directory.
pub fn get_agent_dir() -> PathBuf {
    ...
    dirs::home_dir().unwrap_or_else(|| PathBuf::from(".")).join(format!(".{APP_NAME}"))
}
...
/// Default session directory for a cwd (`~/.pirs/sessions/--<cwd>--`, not created).
...
struct LoadedFile {
    ...
    migrated: bool,
    /// The `seq` the next appended entry gets: one past the highest seen.
    next_seq: u64,
}

/// Number entries 1..n in file order, discarding whatever they carried. Used
/// when entries are copied into a new file, whose numbering starts over.
fn renumber_from_one(entries: &mut [SessionEntry]) {
    for (i, entry) in entries.iter_mut().enumerate() {
        entry.seq = i as u64 + 1;
    }
}

/// Give every entry a `seq`: keep the one the file states, and number the ones
/// without (a file pi wrote) in file order. Returns the next free number.
fn number_entries(entries: &mut [SessionEntry]) -> u64 {
    let mut next = 1u64;
...
fn values_to_loaded(mut values: Vec<Value>) -> Option<LoadedFile> {
    ...
    let next_seq = number_entries(&mut entries);
    Some(LoadedFile { header, entries, migrated, next_seq })
}
...
pub struct SessionManager {
    ...
    /// The `seq` the next appended entry gets.
    next_seq: u64,
}
...
impl SessionManager {
    ...
    /// New session in `session_dir` (default: `~/.pirs/sessions/--<cwd>--/`).
    ...
    fn append_entry(&mut self, kind: EntryKind) -> Result<String> {
        ...
        let entry =
            SessionEntry { id: self.next_id(), parent_id: self.leaf_id.clone(), timestamp: now_iso(), seq, kind };
    ...
    /// The `seq` the next appended entry will get.
    pub fn next_seq(&self) -> u64 {
        self.next_seq
    }

    /// Entries with `seq` strictly greater than `since`, in file order: what
    /// `subscribe … since` replays (D-06).
    pub fn entries_since(&self, since: u64) -> Vec<&SessionEntry> {
    ...
    pub fn append_compaction_full(
        ...
        let seq = self.take_seq();
        let entry = SessionEntry { id: self.next_id(), parent_id: self.leaf_id.clone(), timestamp, seq, kind };
        let id = entry.id.clone();
        self.entries.push(entry);
        let idx = self.entries.len() - 1;
        self.by_id.insert(id.clone(), idx);
    ...
    pub fn fork(&self, leaf_id: &str) -> Result<SessionManager> {
        ...
        // A fork is a new file, so it gets its own numbering from 1.
        renumber_from_one(&mut entries);
    ...
    /// Path of the most recent session whose display name is exactly `name`.
    /// Names are set by `session_info` entries and are not unique, so the
    /// newest match wins.
    pub fn find_by_name(cwd: &str, name: &str, session_dir: Option<&Path>) -> Result<Option<PathBuf>> {
        let name = name.trim();
        Ok(Self::list(cwd, session_dir)?.into_iter().find(|s| s.name.as_deref() == Some(name)).map(|s| s.path))
    }
...
/// Helpers other modules' tests use to redirect `~/.pirs`.
#[cfg(test)]
pub(crate) mod tests_support {
    ...
    fn lock() -> MutexGuard<'static, ()> {
        ...
        // the environment variable is still ours to set.
        LOCK.get_or_init(|| Mutex::new(())).lock().unwrap_or_else(|e| e.into_inner())
    }

    /// Run `body` with `PIRS_HOME` pointing at `home`. Serialised against the
    /// other users of this helper, because the environment is process-wide.
    pub(crate) fn with_pirs_home<T>(home: &Path, body: impl FnOnce() -> T) -> T {
        let _guard = lock();
        let previous = std::env::var_os(super::ENV_AGENT_DIR);
        std::env::set_var(super::ENV_AGENT_DIR, home.join(".pirs"));
        let out = body();
        match previous {
            Some(v) => std::env::set_var(super::ENV_AGENT_DIR, v),
            None => std::env::remove_var(super::ENV_AGENT_DIR),
        }
        out
    }
}
...
mod tests {
    ...
    fn pi_v3_sample_loads_and_unknown_entries_round_trip() {
        ...
        // The header carries no seq.
        let header_rt: Value = serde_json::from_str(&serde_json::to_string(mgr.get_header()).unwrap()).unwrap();
        assert_eq!(header_rt, originals[0]);
        assert!(header_rt.get("seq").is_none());
        for (entry, original) in mgr.get_entries().iter().zip(&originals[1..]) {
            let mut rt: Value = serde_json::from_str(&serde_json::to_string(entry).unwrap()).unwrap();
            // `seq` is ours; everything pi wrote round-trips unchanged.
            assert_eq!(rt["seq"].as_u64(), Some(entry.seq), "seq on the wire for {}", entry.id);
            rt.as_object_mut().unwrap().remove("seq");
    ...
    #[test]
    fn seq_is_assigned_on_append_and_restored_on_reopen() {
        let root = tempfile::tempdir().unwrap();
        let mut mgr = SessionManager::create("/tmp/project", Some(root.path())).unwrap();
        assert_eq!(mgr.next_seq(), 1);
        let u1 = mgr.append_message(user("one")).unwrap();
        mgr.append_message(assistant("two")).unwrap();
        mgr.append_thinking_level_change("high").unwrap();
        assert_eq!(mgr.get_entries().iter().map(|e| e.seq).collect::<Vec<_>>(), [1, 2, 3]);
        assert_eq!(mgr.next_seq(), 4);
        assert_eq!(mgr.get_entry(&u1).unwrap().seq, 1);

        // On the wire, right after `timestamp` and on every entry.
        let file = mgr.get_session_file().unwrap().to_path_buf();
        let lines = read_values(&file);
        assert!(lines[0].get("seq").is_none(), "the header has no seq");
        assert_eq!(lines[1]["seq"], 1);
        assert_eq!(lines[3]["seq"], 3);
        let raw = fs::read_to_string(&file).unwrap();
        assert!(raw.lines().nth(1).unwrap().contains(r#"","seq":1,"message":"#), "{}", raw.lines().nth(1).unwrap());

        // Reopening restores the numbering and continues from max + 1.
        let mut reopened = SessionManager::open(&file).unwrap();
        assert_eq!(reopened.get_entries().iter().map(|e| e.seq).collect::<Vec<_>>(), [1, 2, 3]);
        assert_eq!(reopened.next_seq(), 4);
        reopened.append_message(user("four")).unwrap();
        assert_eq!(reopened.get_entries().last().unwrap().seq, 4);
        assert_eq!(read_values(&file)[4]["seq"], 4);

        // entries_since is the replay window.
        let since: Vec<u64> = reopened.entries_since(2).iter().map(|e| e.seq).collect();
        assert_eq!(since, [3, 4]);
        assert!(reopened.entries_since(4).is_empty());

        // Branching does not reuse a number: seq is file position, not tree position.
        reopened.branch(&u1).unwrap();
        reopened.append_message(user("alt")).unwrap();
        assert_eq!(reopened.get_entries().last().unwrap().seq, 5);
    }

    #[test]
    fn entries_without_seq_are_numbered_in_file_order() {
        let root = tempfile::tempdir().unwrap();
        let file = root.path().join("2024-12-03T14-00-00-000Z_sample.jsonl");
        fs::write(&file, PI_V3_SAMPLE).unwrap();
        let mut mgr = SessionManager::open(&file).unwrap();
        let seqs: Vec<u64> = mgr.get_entries().iter().map(|e| e.seq).collect();
        assert_eq!(seqs, (1..=14).collect::<Vec<u64>>());
        assert_eq!(mgr.next_seq(), 15);
        mgr.append_message(user("after a pi file")).unwrap();
        assert_eq!(mgr.get_entries().last().unwrap().seq, 15);

        // A file that states some seqs keeps them and numbers the rest.
        let mixed = root.path().join("mixed.jsonl");
        fs::write(
            &mixed,
            concat!(
                r#"{"type":"session","version":3,"id":"m","timestamp":"2024-01-01T00:00:00.000Z","cwd":"/p"}"#, "\n",
                r#"{"type":"message","id":"aaaaaaaa","parentId":null,"timestamp":"2024-01-01T00:00:01.000Z","seq":7,"message":{"role":"user","content":"a","timestamp":1}}"#, "\n",
                r#"{"type":"message","id":"bbbbbbbb","parentId":"aaaaaaaa","timestamp":"2024-01-01T00:00:02.000Z","message":{"role":"user","content":"b","timestamp":2}}"#, "\n",
            ),
        )
        .unwrap();
        let mgr = SessionManager::open(&mixed).unwrap();
        assert_eq!(mgr.get_entries().iter().map(|e| e.seq).collect::<Vec<_>>(), [7, 8]);
        assert_eq!(mgr.next_seq(), 9);
    }

    #[test]
    fn a_fork_numbers_its_own_file_from_one() {
        let root = tempfile::tempdir().unwrap();
        let file = root.path().join("2024-12-03T14-00-00-000Z_sample.jsonl");
        fs::write(&file, PI_V3_SAMPLE).unwrap();
        let mgr = SessionManager::open(&file).unwrap();
        let forked = mgr.fork("k1l2m3n4").unwrap();
        let seqs: Vec<u64> = forked.get_entries().iter().map(|e| e.seq).collect();
        assert_eq!(seqs, (1..=seqs.len() as u64).collect::<Vec<u64>>());
        assert_eq!(forked.next_seq(), seqs.len() as u64 + 1);

        let copied = SessionManager::fork_from(&file, "/tmp/elsewhere", Some(root.path())).unwrap();
        let seqs: Vec<u64> = copied.get_entries().iter().map(|e| e.seq).collect();
        assert_eq!(seqs, (1..=14).collect::<Vec<u64>>());
    }

    #[test]
    fn the_agent_dir_follows_pirs_home_and_the_home_directory() {
        let home = tempfile::tempdir().unwrap();
        tests_support::with_pirs_home(home.path(), || {
            assert_eq!(get_agent_dir(), home.path().join(".pirs"));
            assert_eq!(get_sessions_dir(), home.path().join(".pirs").join("sessions"));
            assert_eq!(
                get_default_session_dir_path("/tmp/x"),
                home.path().join(".pirs").join("sessions").join("--tmp-x--")
            );
        });
    }

    #[test]
    fn sessions_map_to_the_protocol_conversation_shape() {
        let root = tempfile::tempdir().unwrap();
        let mut mgr = SessionManager::create("/tmp/project", Some(root.path())).unwrap();
        mgr.append_message(user("q")).unwrap();
        mgr.append_message(assistant("a")).unwrap();
        mgr.set_session_name("named one").unwrap();

        let sessions = SessionManager::list("/tmp/project", Some(root.path())).unwrap();
        assert_eq!(sessions.len(), 1);
        let info = sessions[0].to_conversation_info();
        assert_eq!(info.id, mgr.get_session_id());
        assert_eq!(info.name.as_deref(), Some("named one"));
        assert_eq!(info.cwd.as_str(), "/tmp/project");
        assert_eq!(info.path.as_str(), mgr.get_session_file().unwrap().to_string_lossy());
        assert_eq!(info.updated, sessions[0].modified.timestamp_millis() as u64);

        let found = SessionManager::find_by_name("/tmp/project", "named one", Some(root.path())).unwrap();
        assert_eq!(found.as_deref(), mgr.get_session_file());
        assert!(SessionManager::find_by_name("/tmp/project", "nope", Some(root.path())).unwrap().is_none());
    }
