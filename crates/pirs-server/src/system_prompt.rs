...
//! the wire protocol. Phase 2 assembles the rest of the prompt from the DSL's
//! `[[prompt]]` entries on top of what this builds (D-21).
...
/// was built from (the workspace root above `crates/pirs-server`), or
/// `~/.pirs`. A directory counts as the docs root when it holds
/// `docs/protocol.md`.
pub fn docs_root() -> Option<PathBuf> {
    ...
    candidates.push(crate::session::get_agent_dir());
    candidates.into_iter().map(|c| std::fs::canonicalize(&c).unwrap_or(c)).find(|c| c.join("docs").join("protocol.md").is_file())
}
...
fn docs_section() -> Option<String> {
    ...
    Some(format!(
        "pirs documentation (read only when the user asks about pirs itself, its policy files, tools, sessions, or how to change how it behaves):
- Main documentation: {}
- Additional docs: {}
- Examples: {}
...
- The wire protocol between the pirs server and its clients: {}
- Implementation status: {}",
        root.join("README.md").display(),
        docs.display(),
        examples.display(),
        docs.join("dsl.md").display(),
        docs.join("protocol.md").display(),
...
mod tests {
    ...
    fn the_docs_section_keys_on_protocol_md_and_points_at_the_dsl() {
        ...
        std::env::set_var("PIRS_DOCS_DIR", root.path());
