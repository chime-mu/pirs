...
//!
//! `write` and `edit` report the absolute path they wrote in
//! `ToolResult::details["path"]`, so the loop can emit `fs.changed` for its
//! own writes without parsing tool text.
...
pub fn arg_usize(args: &serde_json::Value, key: &str) -> Option<usize> {
...
pub fn arg_str<'a>(args: &'a serde_json::Value, key: &str) -> anyhow::Result<&'a str> {
    args.get(key)
