...
- [containment.md](containment.md): running the server inside a boundary — `servers.toml`, the bridge command, the container image, and what the boundary does not cover
- `../examples/policy/`: working policy examples, one directory each, with their scripts
- [session-format.md](session-format.md): the session JSONL format — pi's v3 entries plus the per-entry `seq` and the `pirs.*` entries that make the log the event stream
- `../scripts/acceptance/`: one script per phase driving the real binary offline; `scripts/acceptance/README.md` says how to run them
